...
docker compose exec rescuedogs pnpm run type-check    # TypeScript
docker compose exec rescuedogs pnpm run lint           # ESLint
docker compose exec rescuedogs pnpm run format:check   # Prettier
docker compose exec rescuedogs pnpm test              # Unit tests
docker compose exec rescuedogs pnpm run test:coverage # Coverage
docker compose exec rescuedogs pnpm run test:a11y     # Accessibility
```
...
```bash
docker compose exec rescuedogs pnpm run e2e:accessibility
```
...
docker compose exec rescuedogs node scripts/validate-structure.js
```

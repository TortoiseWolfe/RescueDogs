...
**Goal:** Add a Vitest `globalSetup` hook to `pnpm test:rls` that scrubs orphan `*@rescuedogs.test` users + their FK-blocking dependent rows before tests collect, so killed prior runs don't wedge the suite.
...
**Tracks:** [#50](https://github.com/TortoiseWolfe/RescueDogs/issues/50) (Family D1 in `docs/STABILITY-TRACKING.md`)
...
```bash
docker compose exec rescuedogs git checkout -b 050/rls-cleanup-stale
```
...
describe('cleanupStaleScripthammerUsers (#50)', () => {
  ...
  it('deletes the FK chain in correct order per matching user', async () => {
    const client = makeMockClient({
      users: [
        { id: 'user-a-id', email: 'test-user-a@rescuedogs.test' },
        { id: 'user-b-id', email: 'test-user-b@rescuedogs.test' },
      ],
    });
  ...
  it('ignores users whose email does not match @rescuedogs.test', async () => {
    const client = makeMockClient({
      users: [
        { id: 'prod-id', email: 'real-user@example.com' },
        { id: 'admin-id', email: 'admin@rescuedogs.com' }, // .com, not .test
        { id: 'sh-id', email: 'test-user-a@rescuedogs.test' },
      ],
    });
  ...
  it('continues to subsequent steps when a DELETE fails (best-effort)', async () => {
    const client = makeMockClient({
      users: [{ id: 'user-x', email: 'test-user-a@rescuedogs.test' }],
  ...
  it('is a no-op when no @rescuedogs.test users exist', async () => {
    const client = makeMockClient({
...
```bash
docker compose exec rescuedogs pnpm vitest run tests/unit/rls-cleanup.test.ts
```
...
const SCRIPTHAMMER_TEST_DOMAIN = '@rescuedogs.test';
...
export async function cleanupStaleScripthammerUsers(
  ...
  // 1. List all auth users; filter to the rescuedogs.test domain.
...
```bash
docker compose exec rescuedogs git add tests/unit/rls-cleanup.test.ts tests/rls/__setup__/cleanup-stale-impl.ts
docker compose exec rescuedogs git commit -m "feat(rls): cleanup-stale impl + 4 regression cases
...
for every *@rescuedogs.test user. Best-effort: errors are logged
...
- No-op when no rescuedogs.test users exist.
...
```ts
/**
 * Vitest globalSetup hook that scrubs stale *@rescuedogs.test users +
...
```bash
docker compose exec rescuedogs git add tests/rls/__setup__/cleanup-stale.ts
docker compose exec rescuedogs git commit -m "feat(rls): globalSetup wrapper for cleanup-stale
...
export default defineConfig({
  test: {
    ...
    testTimeout: 30000,
    hookTimeout: 30000,
    // Scrub stale *@rescuedogs.test users + orphan FK rows before any
...
```bash
docker compose exec rescuedogs pnpm test:rls 2>&1 | tail -30
```
...
```bash
docker compose exec rescuedogs git add vitest.rls.config.ts
docker compose exec rescuedogs git commit -m "feat(rls): wire cleanup-stale globalSetup into pnpm test:rls

Killed prior runs that left orphan payment_intents/subscriptions/profiles
referencing *@rescuedogs.test auth users would wedge createTestUser
...
VALUES ('00000000-0000-4000-8000-000000050050', 'test-user-a@rescuedogs.test', 'fake');

INSERT INTO payment_intents (template_user_id, amount, currency, type, customer_email)
VALUES ('00000000-0000-4000-8000-000000050050', 1000, 'usd', 'one_time', 'test-user-a@rescuedogs.test');
```

- [ ] **Step 3: Run pnpm test:rls and observe**

```bash
docker compose exec rescuedogs pnpm test:rls 2>&1 | head -10
```
...
Adds a Vitest \`globalSetup\` to \`pnpm test:rls\` that scrubs orphan \`*@rescuedogs.test\` users and their FK-blocking dependent rows (\`payment_intents\`, \`subscriptions\`, \`user_profiles\`) before any RLS test file collects. The fixture's \`createTestUser\` retry path remains as the second line of defense; this hook just prevents the FK-block that defeated it on 2026-04-26 (and required manual psql cleanup).
...
```bash
docker compose exec rescuedogs git checkout main
git fetch --prune origin
git merge --ff-only origin/main
...
- Production-safety filter (`@rescuedogs.test` only): Task 3 step 1 + Task 2 test 2 ✓

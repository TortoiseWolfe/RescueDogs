...
1. Click "New project"
2. Fill in details:
   - **Name**: `rescuedogs-payments` (or your project name)
...
Edit `/home/turtle_wolfe/repos/RescueDogs/.env` and add:
...
```bash
docker compose exec rescuedogs supabase --version
...
docker compose exec rescuedogs supabase link --project-ref $SUPABASE_PROJECT_REF --password $SUPABASE_DB_PASSWORD
```
...
```bash
# Push migrations
docker compose exec rescuedogs supabase db push
...
```bash
# Generate types file
docker compose exec rescuedogs sh -c 'supabase gen types typescript --linked > /app/src/lib/supabase/types.ts'

# Verify file created
docker compose exec rescuedogs ls -lh /app/src/lib/supabase/types.ts
```
...
1. Click **Create App**
2. App Name: `RescueDogs Payments`

...
# Color palette - match RescueDogs but slightly different for distinction
...
COLOR_BASE="colour239"       # Darker gray (distinguishes from rescuedogs)
...
case "$CLIENT" in
  stw)
    # SpokeToWork repos are siblings to RescueDogs
    CLIENT_DIR="$(dirname "$PROJECT_DIR")/SpokeToWork-MVP"
    if [ ! -d "$CLIENT_DIR" ]; then
      echo "Warning: $CLIENT_DIR not found, using RescueDogs dir"
...
# Status bar styling (slightly different from rescuedogs for visual distinction)

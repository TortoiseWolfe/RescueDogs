#!/usr/bin/env bash
# =============================================================================
# RescueDogs Rebrand Script
# =============================================================================
# Automates rebranding of the RescueDogs template to a new project identity.
...
#   --preserve-attribution Keep RescueDogs attribution link in Footer
...
ORIGINAL_NAME="RescueDogs"
ORIGINAL_NAME_LOWER="rescuedogs"
...
# Count RescueDogs references to detect if already rebranded
count_references() {
    local count
...
#
# A fresh RescueDogs clone contains hundreds of "RescueDogs" references
...
# Update CNAME file (replace rescuedogs domain with new project domain)
update_cname() {
    local cname_file="$REPO_ROOT/public/CNAME"

    if [ -f "$cname_file" ]; then
        # Check if it's a custom domain (not rescuedogs.com)
        local domain
        domain=$(cat "$cname_file" 2>/dev/null || echo "")

        if [[ "$domain" == *"rescuedogs"* ]] || [ -z "$domain" ]; then
...
scaffold_themes() {
    ...
    if grep -q "rescuedogs-dark" "$css_file" 2>/dev/null; then
        if [ "$DRY_RUN" = true ]; then
            log_verbose "[DRY-RUN] Would rename theme references in globals.css"
        else
            sed "${SED_INPLACE[@]}" "s|rescuedogs-dark|${SANITIZED_NAME}-dark|g" "$css_file"
            sed "${SED_INPLACE[@]}" "s|rescuedogs-light|${SANITIZED_NAME}-light|g" "$css_file"
            sed "${SED_INPLACE[@]}" "s|RescueDogs Dark Theme|${DISPLAY_NAME} Dark Theme|g" "$css_file"
            sed "${SED_INPLACE[@]}" "s|RescueDogs Light Theme|${DISPLAY_NAME} Light Theme|g" "$css_file"
            log_verbose "Renamed theme blocks: rescuedogs-* → ${SANITIZED_NAME}-*"
        fi
        ((FILES_MODIFIED++)) || true
    fi
    ...
    if [ -f "$theme_script" ] && grep -q "rescuedogs-dark" "$theme_script" 2>/dev/null; then
        if [ "$DRY_RUN" = true ]; then
            log_verbose "[DRY-RUN] Would update ThemeScript.tsx theme names"
        else
            sed "${SED_INPLACE[@]}" "s|rescuedogs-dark|${SANITIZED_NAME}-dark|g" "$theme_script"
            sed "${SED_INPLACE[@]}" "s|rescuedogs-light|${SANITIZED_NAME}-light|g" "$theme_script"
    ...
    if [ -f "$preview_file" ] && grep -q "rescuedogs-dark" "$preview_file" 2>/dev/null; then
        if [ "$DRY_RUN" = true ]; then
            log_verbose "[DRY-RUN] Would update .storybook/preview.tsx theme names"
        else
            sed "${SED_INPLACE[@]}" "s|rescuedogs-dark|${SANITIZED_NAME}-dark|g" "$preview_file"
            sed "${SED_INPLACE[@]}" "s|rescuedogs-light|${SANITIZED_NAME}-light|g" "$preview_file"
...
main() {
    ...
    echo ""
    echo "========================================="
    echo "  RescueDogs Rebrand Script v${VERSION}"
    echo "========================================="
    echo ""

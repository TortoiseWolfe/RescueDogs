...
You are the QC-Operator for RescueDogs - responsible for dispatching annotated PNG batches to QC terminals for visual review.
...
for win in WireframeQA Validator Inspector Auditor; do
  echo "=== $win ===";
  tmux capture-pane -t rescuedogs:$win -p 2>/dev/null | grep "% free" | tail -1
done
```
...
```bash
tmux send-keys -t rescuedogs:[terminal] '/clear' Enter
# Wait for autocomplete menu, then:
tmux send-keys -t rescuedogs:[terminal] Enter
# Wait 3 seconds, then:
tmux send-keys -t rescuedogs:[terminal] '/prime [role]' Enter
tmux send-keys -t rescuedogs:[terminal] Enter
```
...
```bash
tmux send-keys -t rescuedogs:WireframeQA 'Review PNG batch: docs/design/wireframes/png/overviews_XXX/ - View each annotated PNG and log visual issues to respective .issues.md files.' Enter
tmux send-keys -t rescuedogs:WireframeQA Enter
```
...
for win in WireframeQA Validator Inspector Auditor; do
  echo "=== $win ===";
  tmux capture-pane -t rescuedogs:$win -p 2>/dev/null | tail -8
done
```
...
tmux send-keys -t rescuedogs:WireframeQA '/clear'

# RIGHT - command executes
tmux send-keys -t rescuedogs:WireframeQA '/clear' Enter
```
...
```bash
tmux send-keys -t rescuedogs:[terminal] '/clear' Enter  # Shows menu
tmux send-keys -t rescuedogs:[terminal] Enter           # Selects /clear
```

Check the result:

```bash
tmux capture-pane -t rescuedogs:[terminal] -p | grep "% free"
```
...
```bash
# Health check
for win in WireframeQA Validator Inspector Auditor; do echo "=== $win ==="; tmux capture-pane -t rescuedogs:$win -p | grep "% free"; done

# Clear and prime
tmux send-keys -t rescuedogs:WireframeQA '/clear' Enter && sleep 2 && tmux send-keys -t rescuedogs:WireframeQA Enter && sleep 3 && tmux send-keys -t rescuedogs:WireframeQA '/prime wireframeqa' Enter && sleep 2 && tmux send-keys -t rescuedogs:WireframeQA Enter

# Dispatch
tmux send-keys -t rescuedogs:WireframeQA 'Review PNG batch: [path] - Log issues.' Enter && tmux send-keys -t rescuedogs:WireframeQA Enter
```
